...
const InfiniteScrollCarousel: React.FC = () => {
  ...
  return (
    <motion.div
      className="relative mx-auto max-w-7xl w-full overflow-hidden pt-12 my-4"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      >
        {[...logos, ...logos, ...logos].map((logo, index) => (
          <motion.div
            ...
            whileTap={{ scale: 0.9 }}
          >
            <span className="text-4xl md:text-5xl md:mr-24 mr-12 hover:cursor-pointer font-bold text-gray-800">{logo.name}</span>
          </motion.div>
        ))}
      </motion.div>
    </motion.div>
